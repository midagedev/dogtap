...
APM forwarding is explicitly deferred. Until G1 APM fixture evidence and a follow-up contract decision exist, Dogtap may inspect APM intake payloads but should not forward them through this package. Production deployments that need trace forwarding should keep the Datadog Agent or OpenTelemetry Collector on the primary forwarding path.
...
- APM forwarding remains unsupported until fixture-backed behavior is reviewed.
